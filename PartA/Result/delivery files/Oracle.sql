...
--FT_MAINTENANCE_EVENTS is the Fact table of maintenance_events star schema:
...
--DT_TIME_3 is the dimentional table connected to the fact table(FT_MAINTENANCE_EVENTS) in maintenance_events star schema.
...
--DT_AIRCRAFT_3 is the dimentional table connected to the fact table(FT_MAINTENANCE_EVENTS) in maintenance_events star schema.
...
--DT_TYPE_OF_MAINTENANCE_EVENT is the dimentional table connected to the fact table(FT_MAINTENANCE_EVENTS) in maintenance_events star schema.
...
--FT_WORK_ORDERS is the Fact table of work_orders star schema:
...
--DT_TIME_2 is the dimentional table connected to the fact table(FT_WORK_ORDERS) in work_orders star schema.
...
--DT_TYPE_OF_WORK_ORDER is the dimentional table connected to the fact table(FT_WORK_ORDERS) in work_orders star schema.
...
--DT_AIRCRAFT_2 is the dimentional table connected to the fact table(FT_WORK_ORDERS) in work_orders star schema.
...
--DT_TYPE_OF_REPORTING_PERSON is the dimentional table connected to the fact table(FT_WORK_ORDERS) in work_orders star schema.
...
CREATE MATERIALIZED VIEW MAINTENANCE_EVENTS_VIEW ENABLE QUERY REWRITE AS
SELECT
m.MAINTENANCE_REFERENCE_ID,
m.AIRCRAFT_REGISTRATION_CODE,
m.TIMESTAMP,
m.DURATION,
m.TYPE_OF_OPRT_INTR_DLY_OR_SFTY,
m.TYPE_OF_ARCR_OUT_OF_SRVC_SCHD,
t.TIME,
t.MONTH,
t.YEAR,
a.AIRCRAFT,
a.AIRCRAFT_REGISTRATION_CODE,
a.AIRCRAFT_MODEL,
e.TYPE_OF_MAINTENANCE_EVENT,
e.TYPE_OF_MNTN_EVNT_OPRT_INTR_O,
e.TYPE_OF_OPRT_INTR_DLY_OR_SFTY,
e.TYPE_OF_ARCR_OUT_OF_SRVC_SCHD
FROM FT_MAINTENANCE_EVENTS m, DT_TIME_3 t, DT_AIRCRAFT a, DT_TYPE_OF_MAINTENANCE_EVENT e
WHERE m.ID_TIME = t.ID_TIME AND m.ID_AIRCRAFT = a.ID_AIRCRAFT AND m.ID_TYPE_OF_MAINTENANCE_EVENT = e.m.ID_TYPE_OF_MAINTENANCE_EVENT
GROUP BY 
m.MAINTENANCE_REFERENCE_ID,
m.AIRCRAFT_REGISTRATION_CODE,
m.TIMESTAMP,
...
-----------------------------------------------------------------------------------------------------------------------------
--c) for computing RRh, RRc, PRRh, PRRc, MRRh, MRRc per aircraft (also per model, per manufacturer) per month (also per year)

CREATE MATERIALIZED VIEW MAINTENANCE_EVENTS_VIEW ENABLE QUERY REWRITE AS
SELECT
w.WORK_ORDER_ID,
w.AIRCRAFT_REGISTRATION_CODE,
w.EXECUTION_DATE,
w.EXECUTION_PLACE,
w.TYPE_OF_WORK_ORDR_FRCS_ORDR_O,
w.TYPE_OF_RPRT_PRSN_MNTN_OR_PLT,
t.TIME,
t.MONTH,
t.YEAR,
a.AIRCRAFT,
a.AIRCRAFT_REGISTRATION_CODE,
a.AIRCRAFT_MODEL,
a.MANUFACTURER_RGST_CODE,
a.MANUFACTURER,
o.TYPE_OF_WORK_ORDER,
o.TYPE_OF_WORK_ORDR_FRCS_ORDR_O,
e.TYPE_OF_MAINTENANCE_EVENT,
e.TYPE_OF_MNTN_EVNT_OPRT_INTR_O,
e.TYPE_OF_OPRT_INTR_DLY_OR_SFTY,
e.TYPE_OF_ARCR_OUT_OF_SRVC_SCHD,
r.TYPE_OF_REPORTING_PERSON,
r.TYPE_OF_RPRT_PRSN_MNTN_OR_PLT
FROM FT_WORK_ORDERS w, DT_TIME_2 t, DT_AIRCRAFT a, DT_TYPE_OF_WORK_ORDER o, DT_TYPE_OF_REPORTING_PERSON r
WHERE w.ID_TIME = t.ID_TIME AND w.ID_AIRCRAFT = a.ID_AIRCRAFT AND w.ID_TYPE_OF_WORK_ORDER = o.ID_TYPE_OF_WORK_ORDER AND w.ID_TYPE_OF_REPORTING_PERSON = r.ID_TYPE_OF_REPORTING_PERSON
GROUP BY 
w.WORK_ORDER_ID,
w.AIRCRAFT_REGISTRATION_CODE,
w.EXECUTION_DATE,
